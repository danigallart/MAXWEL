subroutine derivatives_nodal
    ...
    integer :: ii,kk,i,j,k, bound, index1, index2
    complex*16 :: DSOLX(NE), DSOLY(NE)
    complex*16 :: inv_tensor_xx,inv_tensor_xy, &
                  inv_tensor_yx,inv_tensor_yy
    complex*16 :: determinant_tensor, jx, jy

    double precision :: tangent_vec(2)
    
    double precision :: r
    
    double precision :: min_tol, max_tol, pol_coeff
    
    allocate(plane_field_x(NP),plane_field_y(NP))
    ...
    do ii = 1, NP
        plane_field_x(ii) = cmplx(0.0,0.0)
        plane_field_y(ii) = cmplx(0.0,0.0)
        DSOLX(ii) = cmplx(0.0,0.0)
        DSOLY(ii) = cmplx(0.0,0.0)
    enddo
    ...
    do kk = 1,NE
        ...
        do ii = 1, nodpel
            ...
            do k = 1, Ngauss
            DSOLX(kk) = DSOLX(kk) + u_tot(i)*DPHIX(ii,k)/(Ngauss*Ngauss)
            DSOLY(kk) = DSOLY(kk) + u_tot(i)*DPHIY(ii,k)/(Ngauss*Ngauss)
            enddo
        ...
        do i= 1, NP
            if (boundary(i) == 6) then
                
                jx = sqrt(current_density1_x**2+current_density1_y**2) * tangent_vec(1)
                jy = sqrt(current_density1_x**2+current_density1_y**2) * tangent_vec(2)
                
            else if (boundary(i) == 7) then
                
                jx = sqrt(current_density2_x**2+current_density2_y**2) * tangent_vec(1)
                jy = sqrt(current_density2_x**2+current_density2_y**2) * tangent_vec(2)
            ...
            plane_field_x(i) = plane_field_x(i) + (inv_tensor_xx*(DSOLY(ii)-jx) + inv_tensor_xy*(-DSOLX(ii)-jy))/(ij*omg*pol_coeff)
            plane_field_y(i) = plane_field_y(i) + (inv_tensor_yx*(DSOLY(ii)-jx) + inv_tensor_yy*(-DSOLX(ii)-jy))/(ij*omg*pol_coeff)
    ...
    end subroutine derivatives_nodal