...
sumideroPH= 6.0200e+05  ! At/mmm3 equivale a PH=12
   ...
   DO JEL=1,nelements
         ...
         do inode=1,nodpel
            ...
            if(  vec_tierra(ipoin)/=-1 ) then
              ...
              do jnode=1,nodpel
                 ESM(inode,jnode)=0.0
                 EF (jnode)=EF(jnode)-ESM(jnode,inode)*sumir*fuentePH
                 ESM(jnode,inode)=0.0
              end do
              ESM(inode,inode)= adiag
              EF(inode)       = adiag* sumir*fuentePH
            end if
            if(  vec_poten(ipoin)/=-1 ) then
              adiag=ESM(inode,inode)
              do jnode=1,nodpel
                 ESM(inode,jnode)=0.0
                 EF (jnode)=EF(jnode)-ESM(jnode,inode)* font*sumideroPH
                 ESM(jnode,inode)=0.0
              end do
              ESM(inode,inode)= adiag
              EF(inode)       = adiag* font*sumideroPH 
            end if
        end do
...
!     WRITE(6,'(A25,i3,2e15.5)') 'Ph: iteraciones de tiempo ',nconta,error,Ph_(5000)  
   ...
   if(mod(nconta,NCOTA)==0) then
     WRITE(unit_cont,'(A25,i3,2e15.5)') 'Oh: iteraciones de tiempo  ',nconta,error,dtime 
     WRITE(6,'(A25,i3,2e15.5)') 'Oh: iteraciones de tiempo  ',nconta,error,dtime  
...
deallocate(Ph0,rhs_aux)

end subroutine OH_evol_2d
